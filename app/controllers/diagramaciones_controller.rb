class DiagramacionesController < ApplicationController
	before_filter :signed_in_user
	def index
		xuser = current_user
		if signed_granted?(xuser.id, 'diagramaciones', 'I')
			#@deboxionals = Datadeboxionale.group_by_year
  ...
  # GET /userdefaults/1.json
	...
	def new
		xuser = current_user
		if signed_granted?(xuser.id, 'diagramaciones', 'N')
	...
	def edit
		xuser = current_user
		if signed_granted?(xuser.id, 'diagramaciones', 'E')
			@deboxional= Datadeboxionale.find(params[:id])
			@deboxionales = Datadeboxionale.where("anio = '#{@deboxional.anio}'").all
			pdf = PdfdiagramaciondeboxionalPdf.new(@deboxionales)
